...
if __name__ == '__main__':
    ...
    train = [(one_hot(tensor(u).cuda(), n_users).float(),
              one_hot(tensor(c[type][m]).cuda(), n_entities+n_movies).float(), rating)
             for u, m, rating, type in train]
    test = [(one_hot(tensor(u).cuda(), n_users).float(),
             one_hot(tensor(c[type][m]).cuda(),n_entities+n_movies).float(), rating)
            for u, m, rating, type in test]
